...
class Grafo:
    ...
    def vertice_esta_nao_visitado(self, c):
    ...
    def bfs(self, vertice_inicial):
        ...
        for v in fila:
            vertice = fila.pop(0)
            adjacencias = self.obter_adjacencias(vertice)
            for (vizinho_do_vertice, peso) in adjacencias:
                if self.vertice_esta_nao_visitado(vizinho_do_vertice.get_indice()):
                    self.explorar_aresta(Aresta(vertice, vizinho_do_vertice, peso))
                    fila.append(vizinho_do_vertice)
                    self.marcar_como_visitado(vizinho_do_vertice.get_indice())
                    print(f'{vizinho_do_vertice.get_indice()}', end=' ')
                else:
                    if self.esta_aresta_eh_nao_explorada(Aresta(vertice, vizinho_do_vertice, peso)):
                        self.explorar_aresta(Aresta(vertice, vizinho_do_vertice, peso))
    ...
    def esta_aresta_eh_nao_explorada(self, aresta):
        v1 = aresta.get_origem().get_indice
        v2 = aresta.get_destino().get_indice
        p = aresta.get_peso()
